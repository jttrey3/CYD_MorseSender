                       ...
                       void (*target_init)(void))
{
    if(*target == NULL)
        target_init();
    // spd=0: instant switch — the LVGL 9.1 animation timer does not advance reliably
    // with this loop structure, so we bypass animation to get a synchronous transition.
    lv_screen_load_anim(*target, fademode, 0, 0, false);
}
